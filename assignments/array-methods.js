...
let firstNamesAllCaps1 = [];
runners.forEach(person => firstNamesAllCaps1.push(person.first_name.toUpperCase()));
console.log(firstNamesAllCaps1);

let firstNamesAllCaps = [];
runners.map(data => {
  return firstNamesAllCaps.push(data.first_name.toUpperCase());
})
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter(data => data.shirt_size === "L")
runners.filter(function(data) {
